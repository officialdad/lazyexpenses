...
- **Editing `parse.py` invalidates the whole parse cache — and since #115 so does a `pdfplumber`
  bump** (`PARSE_VER` = hash of the file **+** the installed `pdfplumber` version, read from
  `importlib.metadata`). The library half is load-bearing: `parse.py` reads word geometry, not
  text, so a bump can move `top` values against a fixed `ytol` and change what `parse_statement`
  returns for identical PDF bytes. Hashing only the file left every entry valid across such a bump,
  which made the bump untestable locally (all-VERIFIED is a replay) and invisible in prod. Note
  `pdfplumber` pins `pdfminer.six` exactly, so its version covers the whole extraction stack. On the
  ...
  **On a `pdfplumber` bump, `rm -rf /data/cache` first** — rejected entries are ignored, never
  deleted, so they sit on the PVC forever:
...
python tests/test_parse_cache.py      # plain-assert tests for cached_parse (hit/miss, parse.py version-bust, pdfplumber version-bust, corrupt-fallback; prints OK)
